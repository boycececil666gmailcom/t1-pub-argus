...
# 今日のアクティビティレポート（端末表示 + ~/.argus/reports/ に .txt 保存）
python src/main.py report

# 指定日のレポート
python src/main.py report --date 2026-03-15

# 今週のレポート（DB と同じ保存場所に .txt）
...
デフォルトでは `~/.argus/` に保存されます（環境変数 `ARGUS_DATA` で変更可）。

- `argus.db` — SQLite（5 秒ごとのスナップショットが 1 行）
- `reports/` — `report` / `week` コマンドのテキスト出力（`daily-YYYY-MM-DD.txt`、`weekly-YYYY-MM-DD.txt` は週の月曜日）
