...
Argus を設計するにあたり、システムエンジニアの流れに従いました。

### 機能設計 — 何を作るかを決めた

**常駐型のタイムトラッカー**という需求を特定し、以下の設計方針を決定しました：

- フォアグラウンドウィンドウを自動検出・カテゴリ分類
- データを SQLite に保存し、手元だけで完結
- `argus tui` 一発でダッシュボード起動＋トラッキング開始
- ログイン時に自動起動し、ユーザーが意識せず記録
- 6言語・12テーマ対応で幅広い層にアピール
...
| ウィンドウ追跡 | 5 秒間隔で常駐記録 |
| アイドル検出 | 無操作時は記録をスキップ |
| プライバシー優先 | データはすべてローカルのみ |
| 軽量 | 一般的なデスクトップで CPU 1% 未満 |
